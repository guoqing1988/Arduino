...
SparseNeoMatrix::SparseNeoMatrix(int w, int h, uint8_t pin,
  uint8_t matrixType, neoPixelType ledType) : Adafruit_GFX(w, h),
  Adafruit_NeoPixel(((2 * w) - 1) * h, pin, ledType), type(matrixType), matrixWidth(w),
...
uint16_t SparseNeoMatrix::Color(uint8_t r, uint8_t g, uint8_t b) {
...
void SparseNeoMatrix::setPassThruColor(uint32_t c) {
  passThruColor = c;
  passThruFlag  = true;
}

// Call without a value to reset (disable passthrough)
void SparseNeoMatrix::setPassThruColor(void) {
  passThruFlag = false;
}

void SparseNeoMatrix::drawPixel(int16_t x, int16_t y, uint16_t color) {
  ...
  if(1) { // Custom X/Y remapping function
    pixelOffset = (((_width * 2)-1) * y);

    if((y % 2) == 0) {
      pixelOffset += (x * 2);
    }
    else {
      pixelOffset += (((_width * 2) - 2) - (x * 2));
    }
...
void SparseNeoMatrix::fillScreen(uint16_t color) {
  uint16_t i, n;
  uint32_t c;
...
void SparseNeoMatrix::setRemapFunction(uint16_t (*fn)(uint16_t, uint16_t)) {
  remapFn = fn;
}
